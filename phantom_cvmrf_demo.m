...
par_maps = ...
    patternMatching(phantom_4d, dict, dict_lut, phantom_brain_mask_3d);

pattern_matching_time = toc(timer_start(3));

phantom_cvmrf_time = toc(timer_start(1));
...
results.phantom_cvmrf_time = phantom_cvmrf_time;

results_file = 'phantom_cvmrf_demo_results.mat';
...
fig(1) = figure; 
imagesc(par_maps.R2s_map(:, :, phantom_image_idx)); 
colormap('hot'); 
colorbar;  
...
fig(2) = figure; 
imagesc(par_maps.deltafs_map(:, :, phantom_image_idx)); 
colormap('parula'); 
colorbar; 
axis image off; 
